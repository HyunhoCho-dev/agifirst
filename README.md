...
실행 파일을 다운로드할 때 브라우저에서 다음과 같은 보안 경고가 나타날 수 있습니다:

> ⚠️ "AGIfirst.exe은(는) 일반적으로 다운로드되지 않습니다. 열기 전에 AGIfirst.exe를(을) 신뢰하는지 확인합니다."

**이는 정상적인 현상입니다.** 이 프로그램은 아직 코드 서명 인증서가 없어 브라우저가 경고를 표시합니다.

### 다운로드 계속하기

각 브라우저에서 다운로드를 계속하는 방법:
...
2. **다운로드 빈도**: 아직 많은 사용자가 다운로드하지 않아 브라우저가 "알려지지 않은" 파일로 인식합니다
