...
**좋은 소식!** Windows용은 ZIP 파일로 배포되므로 대부분의 브라우저에서 문제없이 다운로드됩니다.

만약 브라우저 경고가 나타나면:
- **Chrome**: 다운로드 바에서 파일 옆의 "^" 버튼을 클릭하고 "보관" 또는 "유지" 선택
- **Edge**: 다운로드 알림에서 "계속 유지" 버튼 클릭
- **Firefox**: 다운로드 목록(Ctrl+J)에서 파일을 찾아 실행

### Windows Defender 경고 (압축 해제 후 실행 시)

ZIP 파일 압축 해제 후 `AGIfirst.exe`를 실행할 때 Windows Defender SmartScreen 경고가 나타날 수 있습니다:
...
1. **코드 서명 인증서 부재**: 유료 인증서($300-500/년)가 없어 Windows SmartScreen이 경고를 표시합니다
2. **다운로드 빈도**: 아직 많은 사용자가 다운로드하지 않아 "알려지지 않은" 앱으로 인식됩니다
...
- 프로그램은 100% 로컬에서 실행됩니다
- 외부 서버로 데이터가 전송되지 않습니다 (Groq API 제외)
- 소스 코드를 직접 검토하고 빌드할 수 있습니다

의심스러운 경우, 소스 코드에서 직접 빌드하여 사용할 수 있습니다 (위의 "방법 2: 소스 코드로 실행" 참조).

### 왜 ZIP 파일로 배포하나요?

Chrome과 Edge가 exe 파일을 직접 다운로드하면 "일반적으로 다운로드되지 않음" 경고로 차단하는 경우가 많습니다. ZIP 파일로 압축하면 이러한 차단을 피할 수 있어 사용자 경험이 개선됩니다.
